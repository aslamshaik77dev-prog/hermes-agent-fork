...
class CLICommandsMixin:
    ...
    def _handle_update_command(self) -> bool:
        """Handle /update — update AS AI PA to the latest version.
        ...
        if is_managed():
            print(f"  ✗ {format_managed_message('update AS AI PA')}")
            return False
        ...
        choices = [
            ("once", "Update Now", "exit the current session and update AS AI PA"),
            ("cancel", "Cancel", "keep the current session"),
        ]
        raw = self._prompt_text_input_modal(
            title="⚕  Update AS AI PA",
